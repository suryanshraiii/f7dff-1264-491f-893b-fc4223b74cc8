...
export function getDescription(p: Product): string {
  ...
  const recoLine =
    p.recommendation === "wait"
      ? `Our AI suggests waiting around ${p.waitDays} days — we expect a price drop of about ${formatINR(p.predictedDrop || 2000)} (confidence ${p.confidence}%).`
      : p.recommendation === "buy_online"
      ? `This is a strong buy-online moment — our model is ${p.confidence}% confident the current online deal is the best you'll see in the near term.`
      : `Best price right now is at offline retailers near you — confidence ${p.confidence}%. Walk-ins often unlock extra bundle perks.`;
  return `The ${p.name} is one of the most-tracked products in our ${p.category} catalog, with ${p.reviews.toLocaleString(
    "en-IN",
  )} reviews and an average rating of ${p.rating}/5. Headline strengths include ${p.features
    .slice(0, 3)
    .join(", ")}. It currently sells for ${formatINR(p.price)} — a saving of ${formatINR(
    p.originalPrice - p.price,
  )} versus its launch price of ${formatINR(p.originalPrice)}. ${recoLine}`;
}

export function buildSpecSheet(p: Product): string {
  const savings = p.originalPrice - p.price;
  const pct = Math.round((savings / p.originalPrice) * 100);
  const lines = [
    "PRICELY — PRODUCT SPEC SHEET",
    "=".repeat(60),
    "",
    `Product : ${p.name}`,
    `Category: ${p.category}`,
    `Rating  : ${p.rating}/5  (${p.reviews.toLocaleString("en-IN")} reviews)`,
    "",
    "PRICING",
    "-".repeat(60),
    `Current price : ${formatINR(p.price)}`,
    `Launch price  : ${formatINR(p.originalPrice)}`,
    `You save      : ${formatINR(savings)} (${pct}% off)`,
    `Lowest ever   : ${formatINR(p.lowestPrice)}`,
    `Highest ever  : ${formatINR(p.highestPrice)}`,
    "",
    "KEY FEATURES",
    "-".repeat(60),
    ...p.features.map((f) => `  • ${f}`),
    ...
    "BADGES",
    "-".repeat(60),
    ...(p.badges.length ? p.badges.map((b) => `  • ${b}`) : ["  • —"]),
    "",
    "AI RECOMMENDATION",
    "-".repeat(60),
    `Verdict    : ${
      p.recommendation === "wait"
        ? `Wait ~${p.waitDays} days for a better deal`
        : p.recommendation === "buy_online"
        ? "Buy online now"
        : "Buy from a nearby offline store"
    }`,
    `Confidence : ${p.confidence}%`,
    `Predicted drop: ${formatINR(p.predictedDrop || 0)}`,
    "",
    "DESCRIPTION",
    "-".repeat(60),
    getDescription(p),
    "",
    "=".repeat(60),
    `Generated by Pricely on ${new Date().toLocaleString("en-IN")}`,
  ];
  return lines.join("\n");
}
